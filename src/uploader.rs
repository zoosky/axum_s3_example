...
impl UploadService {
    ...
    pub async fn upload<'a>(&self, field: Field<'a>) -> Result<String, StatusCode> {
        let orig_filename = field.file_name().unwrap_or("file").to_owned();
        let mimetype = field
            .content_type()
        ...
        self.bucket
            .put_object_stream_with_content_type(&mut reader, &tmp_filename, &mimetype)
            .await
            .map_err(|err| {
        ...
        if let Some(digest) = Arc::into_inner(digest).and_then(|m| m.into_inner().ok()) {
            ...
            match self
                .bucket
                .copy_object_internal(&tmp_filename, &filename)
                .await
            {
    ...
    pub async fn download(&self, filename: &str) -> Result<impl IntoResponse, StatusCode> {
        let stream = self
            .bucket
        ...
        Ok(axum::body::Body::from_stream(stream.bytes))
    }
}
